...
/** Utility class for creating {@link DoubleCheck} types. */
public final class SingleChecks {
  private static final class SingleCheckImpl<T> implements SingleCheck<T> {
    private static final Object UNINITIALIZED = new Object();

    private volatile Provider<T> provider;
    private volatile Object instance = UNINITIALIZED;

    private SingleCheckImpl(Provider<T> provider) {
    ...
    public T get() {
      ...
      if (local == UNINITIALIZED) {
        // provider is volatile and might become null after the check, so get the provider first
  ...
  public static <P extends Provider<T>, T> Provider<T> provider(P provider) {
    ...
    return new SingleCheckImpl<T>(checkNotNull(provider));
  }
}
